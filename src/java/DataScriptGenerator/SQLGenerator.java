...
public class SQLGenerator {
    ...
    public static void generator(int amount) {
        ...
        for (int i = 1; i < amount; i++) {
            tempID = flightID();
            tempFF = firstFour(setRandomFirstFour());
            String[] ft = fromTo();
            System.out.println("('" + tempFF + tempID + "', '" + "FHA" + tempFF + "', '" + makeDate() + "T13:00:00.000Z" + "', " + availableSeats[random.nextInt(4)] + ", " + makePrice(Integer.parseInt(makeDay()), random.nextInt(100) + 1) + ", '" + random.nextInt((250) + 60) + "', '" + ft[0] + "', '" + ft[1] + "'),");

        }
    }

    public static String flightID() {
        //number1 = random.nextInt(9999)+1;
    ...
    public static int setRandomFirstFour() {
