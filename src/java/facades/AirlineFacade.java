...
public class AirlineFacade {
    ...
    public List<Flight> getFlightsFromTo(String from, String to, String date, int tickets) {
        ...
        List<Flight> result = new ArrayList();
        try {
            Query q = em.createQuery("SELECT f FROM Flight f where f.origin=:origin AND and f.destination=:destination AND f.date=:date AND f.numberOfSeats >:tickets", Flight.class);
            q.setParameter("origin", from);
